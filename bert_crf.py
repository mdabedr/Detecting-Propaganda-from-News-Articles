...
#Following on the code from https://www.depends-on-the-definition.com/named-entity-recognition-with-bert/
...
#CRF:
crf = CRF(num_tags=len(tag2idx), batch_first=True)
crf.to("cuda")
...
for e in range(epochs):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        #Compute logits of shape: [batch_size, seq_length, num_tags]
        logits = model(b_input_ids, token_type_ids=None,
                       attention_mask=b_input_mask)

        #Compute loss (negative log likelihood) from CRF
        loss = -1 * crf(logits, b_labels)
    ...
    for batch in valid_dataloader:
        ...
        #Compute loss from CRF
        tmp_eval_loss = -1 * crf(logits, b_labels)

        #Get the CRF prediction over BERT
        pred = crf.decode(logits)
    ...
    pred_tags = [[tags_vals[l_ii] for l_ii in l_i] for l in predictions for l_i in l ]
